<?php 
    ...
    $pastquery=mysqli_query($conn,"select *  from cash_system where user_id='$user' and is_active='n' order by id desc limit 0,1");
	$reports = array();
	$invoice_no = array();
	$date_array = array();
	...
	while($row1 = mysqli_fetch_assoc($pastquery)){
		...
		$sales_start_dt=date('Y-m-d H:i',$row1['login_time']);
		$sales_end_dt=date('Y-m-d H:i',$row1['logout_time']);
		// $end_dt=$row['void_tras'];
		      $saleq="select id,quantity,amount from order_list where status in(0,1,2) and created_on >= '$sales_start_dt'    AND created_on <= '$sales_end_dt' and merchant_id='$user'";
		...
		$date_array_cash = array("opening"=>$opening,"sales"=>$finaltotal,"cash_in"=>$cash_in,"cash_out"=>$cash_out,"void_tras"=>$void_tras,"balancepast"=>$balancepast,"start_dt"=>$sales_start_dt,"end_dt"=>$sales_end_dt);
