...
if(isset($_POST['filter'])){
    ...
    $merchant_sql = mysqli_query($conn, "SELECT DISTINCT(referral_id) referral_id FROM users WHERE mobile_number LIKE '%".$telephone."%' AND referral_id LIKE '%".$referral."%'");
    
    while ($row_referral=mysqli_fetch_assoc($merchant_sql)){
        $referral_id = $row_referral['referral_id'];
        $sql_user = mysqli_query($conn,"select * from users where referral_id = '".$referral_id."' order by user_roles desc limit 1");
        $row = mysqli_fetch_assoc($sql_user);
        $sql = "
            SELECT SUM(month_cf) month_cf, SUM(total_cf) total_cf
            FROM (
            SELECT first_ref, second_ref, third_ref, referred_by, SUM(IF((a.date >= '".$start_dt."') AND (a.date <= '".$end_dt."'), rate, 0)) MONTH, SUM(rate) rate, SUM(IF((a.date >= '".$start_dt."') AND (a.date <= '".$end_dt."'), cf, 0)) month_cf, SUM(cf) total_cf
        	FROM (
        ...
        $item = array("month" => $result_referral['month_cf'], "total" => $result_referral['total_cf'],
    ...
    $merchant_sql = mysqli_query($conn, "SELECT DISTINCT(referral_id) referral_id FROM users");
        ...
        <div class="content-wrapper">
            ...
            <main class="main-wrapper clearfix" style="min-height: 522px;">
                <div class="container-fluid" id="main-content" style="padding-top:25px">
					<h2 class="text-center wallet_h">Community Fund</h2>
					...
					<table class="table table-striped kType_table" id="kType_table">
					    ...
					    <tbody>
					        ...
					        <tr>
					            ...
					            <td><?php echo number_format($array_subscription[$i]['month'], 2);?></td>
					            <td><?php echo number_format($array_subscription[$i]['total'], 2);?></td>
					        </tr>
					        <?php }?>
					    </tbody>
					</table><!--
