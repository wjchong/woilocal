...
if(isset($_POST['filter'])){
    ...
    while ($row=mysqli_fetch_assoc($merchant_sql)){
        ...
        $sql = "
            SELECT sum(cf_month) month, sum(cf_year) rate
            FROM (
            SELECT first_ref, second_ref, third_ref, referred_by, SUM(IF((a.date >= '".$start_dt."') AND (a.date <= '".$end_dt."'), 1, 0)) cf_month, IF(rate IS NULL, SUM(0), SUM(1)) cf_year
                	FROM (
					...
					<table class="table table-striped kType_table" id="kType_table">
					    ...
					    <tbody>
					        ...
					        <tr>
					            ...
					            <td><?php echo 2 * $array_subscription[$i]['month'];?></td>
					            <td><?php echo 2 * $array_subscription[$i]['total'];?></td>
					        </tr>
					        <?php }?>
					    </tbody>
					</table><!--
