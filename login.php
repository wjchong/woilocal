                     ...
                     <div class="resp-tabs-container">
                           <div class="tab-1 resp-tab-content" aria-labelledby="tab_item-0">
                                ...
                                <div class="login-top">	
                                    ...
                                    <select name="countrycode" id="countrycode" class="ct_ctycode" style="padding-left:5px;padding-right:5px;padding-top:10px;padding-bottom:10px;height:40px;">
...
list-style: none;  
    ...
    border-radius: 5px;  
        ...
        <li class="letter" style="width:30%;" onclick="document.getElementById('login_pass').value=''">Clear</li>  
        <li class="letter clearl" onclick="document.getElementById('login_pass').value=document.getElementById('login_pass').value+'4'">4</li>  
        <li class="letter" onclick="document.getElementById('login_pass').value=document.getElementById('login_pass').value+'5'">5</li>  
        <li class="letter" onclick="document.getElementById('login_pass').value=document.getElementById('login_pass').value+'6'">6</li> 
       <li class="letter" style="width:30%;" onclick="document.getElementById('login_pass').value=''">Cancel</li>  
        <li class="letter clearl" onclick="document.getElementById('login_pass').value=document.getElementById('login_pass').value+'7'">7</li>  
...
//alert("sdfds");
		...
		$("input[name='user_roleget']").change(function(){
      		...
      		$('.frmlogin .btn-clock-in').click(function(e) {
	        	    
	        	 save_staff_clock_in_out('save_staff_clock_in', $(this));
	        });
	        $('.frmlogin .btn-clock-out').click(function(e) {
	        	  
	        	 save_staff_clock_in_out('save_staff_clock_out' , $(this));
	        });
    		function save_staff_clock_in_out(action , btn)
    		{
				 	$('.frmlogin div.alert-msg').removeClass('alert-danger alert-success alert-info').html('');
	        	     
	        	    if($("input[name='mobile_number']").val() == "")
				 	{
					   $('.frmlogin div.alert-msg').addClass('alert-danger').html('Please enter mobile no').show();	 
					  return false;
					}
					if($("input[name='password']").val() == "")
				 	{
						 $('.frmlogin div.alert-msg').addClass('alert-danger').html('Please enter password').show();	 
					  return false;
					}
	        	 
				 	var formdata = $('.frmlogin').serialize();
				 	$(btn).attr("disabled", true);
				 	 var loading_txt = '<div class="">Processing your request</div>';
			       
			        $('.frmlogin div.alert-msg').addClass('alert-info').html(loading_txt);
			        $('.frmlogin div.alert-msg').fadeIn();
			       
					var url = 'inc/ajax.php?action='+action;
					var options = {
			            type: 'post',
			            url: url,
			            dataType: 'json',
			            ...
			            success: function(data) {
			                if (data != null) {
			                    ...
			                    if (data.success == true) {
						              ...
						              $('.frmlogin div.alert-msg').addClass('alert-success').html('<strong>'+data.message+'</strong>');	  
						              $('.frmlogin div.alert-msg').fadeIn('slow').delay(2000).fadeOut("slow",'linear', function()
										{
										window.location.reload(true);
										});                            
										
			                    } else {
			                    	 $('.frmlogin div.alert-msg').addClass('alert-danger').html('<strong>'+data.message+'</strong>');
			                         $('.frmlogin div.alert-msg').fadeIn('slow')
			                    }
			                }
			            },
